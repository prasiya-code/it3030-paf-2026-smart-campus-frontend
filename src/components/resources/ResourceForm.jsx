...
const ResourceForm = ({ 
  ...
  const validateForm = () => {
    ...
    if (formData.availabilityStart) {
      ...
      if (startDate < now) {
        newErrors.availabilityStart = 'Cannot set availability start to a past date and time';
      }
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      // Exclude resourceCode - it's auto-generated by backend
      const { resourceCode, ...dataWithoutCode } = formData;
      onSubmit(dataWithoutCode);
    } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      {/* Resource Code - Read Only (Edit Mode Only) */}
      {isEditMode && initialData.resourceCode && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Resource Code
          </label>
          <div className="text-lg font-semibold text-gray-900">
            {initialData.resourceCode}
          </div>
          <p className="text-xs text-gray-500 mt-1">Auto-generated by system</p>
        </div>
      )}
